...
def test_incompatible_search_results_become_auditable_no_results(monkeypatch, tmp_path: Path) -> None:
    ...
    assert download["reason"] == "no_compatible_title_artist_match"
...
def test_exact_match_records_selected_result_metadata(monkeypatch, tmp_path: Path) -> None:
    ...
    class FakeClient:
        def __init__(self, **kwargs):
            ...
            self.exact = SimpleNamespace(source="KugouMusicClient", song_name="空心 (Live)", singers="黄霄雲, 刘端端")
        ...
        def download(self, items):
            ...
            return [SimpleNamespace(save_path=str(path))]
