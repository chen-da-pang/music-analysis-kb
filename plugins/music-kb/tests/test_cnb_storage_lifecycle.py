...
def policy() -> dict:
    return {
        ...
        "preserved_branch_patterns": ["^campaign-results/"],
        "cleanup_branch_patterns": ["^campaign-"],
    ...
    *,
    ...
    def run(command):
        ...
        if "list-branches" in joined:
            rows = [{"name": "main"}, {"name": "campaign-results/kept"}]
            if campaign:
...
def test_inspect_blocks_stale_campaign_assets_and_lfs_volume() -> None:
    ...
    assert result["cleanup_branches"] == ["campaign-old"]
    assert result["cleanup_assets"][0]["id"] == "1"
    assert result["lfs_reclamation_verified"] is False


def test_inspect_accepts_only_runtime_and_result_branches_under_limit() -> None:
    result = MODULE.inspect(
...
def test_cleanup_is_dry_run_without_confirmation() -> None:
    ...
    assert result["plan"]["branches"] == ["campaign-old"]
...
def test_inspect_never_plans_to_delete_a_running_campaign_workspace_branch() -> None:
    ...
    assert result["cleanup_branches"] == []
